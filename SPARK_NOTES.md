...
| **Laguna RC2 NVFP4 exception** | The general Marlin rule above is checkpoint/image-specific and is wrong for Poolside's August RC2 on vLLM 0.25.1 | Leave MoE and attention on `auto`. On dgx4, stock v0.25.1 selected `FLASHINFER_CUTLASS` NVFP4 MoE plus FlashInfer CUTLASS for the BF16-tail experts; generation, tools, and executable code all passed. Treat `MARLIN` as a failed Laguna launch. The 99.7 GB target needs explicit FP8 KV to serve useful context at 0.85 utilization; this is uncalibrated scale-1.0 FP8, so retain the quality warning. | on-box dgx4, 2026-08-09; Poolside RC2 `f8fdfcdc` |
